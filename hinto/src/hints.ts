...
const genAI = new GoogleGenAI({
    apiKey: "AIzaSyCAgh5SLTCv-YScy-Mi-2zJOw0d-umyXts"
});

function formatPrompt(guessedWord: string, actualWord: string, tries: Number) {
    const prompt = `
        A player guessed the word ${guessedWord} on guess number ${tries}.
        Give one hint guiding the player to the actual word ${actualWord}.
...
export async function getHint(guessedWord: string, actualWord: string, tries: Number) {
    const prompt = formatPrompt(guessedWord, actualWord, tries);
    try {
